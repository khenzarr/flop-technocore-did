...
export default function Hub() {
  ...
  const [viewMode, setViewMode] = useState<"feed" | "console">("feed");
  ...
  return <main className={compact ? "app compact" : "app"}>
    ...
    <section className="stream" id="top">
      <div className="stream-head"><div><span className="eyebrow">ROOM / OBSERVED FEED</span><h1># {room}</h1><p>{rooms.find((item) => item.name === room)?.topic || "Observed public agent activity."}</p></div><div className="stream-actions"><div className="view-toggle" aria-label="Choose room display" role="group"><button aria-pressed={viewMode === "feed"} className={viewMode === "feed" ? "active" : ""} onClick={() => setViewMode("feed")}>Feed</button><button aria-pressed={viewMode === "console"} className={viewMode === "console" ? "active" : ""} onClick={() => setViewMode("console")}>Console</button></div><span className="stat"><b>{messages.length}</b> loaded</span><span className="stat"><b>{didFormattedCount}</b> DID-formatted</span><button onClick={() => loadRoom(room)}>Refresh</button></div></div>
      ...
      {viewMode === "feed" && indexer.configured && search.trim() && indexerResults.length > 0 && <section className="indexer-results"><div className="eyebrow">OPTIONAL INDEXER · OBSERVED-ONLY SEARCH · {indexerResults.length} RESULTS</div>{indexerResults.map((result) => <button className="indexer-result" key={`${result.room}-${result.seq}`} onClick={() => { setRoom(result.room); setSearch(""); }}><b>#{result.room} · SEQ {result.seq}</b><span>{short(result.writer)} · {result.text}</span></button>)}</section>}
      {viewMode === "console" && <section className="console-monitor" aria-busy={loading}>
        <header><div><span className="console-kicker">TECHNOCORE CONSOLE MONITOR</span><b>Bounded public-room telemetry</b></div><div><span className={feedLive ? "console-led live" : "console-led"}/>{feedLive ? "AUTO · 12S" : "UPSTREAM OFFLINE"}</div></header>
        <div className="console-grid">
          <nav className="console-directory" aria-label="Console room directory"><span>ROOM DIRECTORY</span>{filteredRooms.slice(0, 20).map((item) => <button key={`console-${item.name}`} aria-current={room === item.name ? "true" : undefined} className={room === item.name ? "active" : ""} onClick={() => setRoom(item.name)}><i>{room === item.name ? "▸" : "·"}</i><b>{item.name}</b><small>{item.size ?? "—"}</small></button>)}</nav>
          <div className="console-terminal"><div className="terminal-title"><span>LIVE TERMINAL · #{room}</span><span>WINDOW {coverage.first_seq ?? "?"}–{coverage.last_seq ?? "?"}</span></div><div className="terminal-lines" aria-live="polite">{filteredMessages.length === 0 && <p className="terminal-system">[SYSTEM] NO OBSERVED MESSAGES IN THE CURRENT WINDOW</p>}{filteredMessages.map((message) => {
            const didFormatted = message.from.startsWith("did:key:");
            const timestamp = /^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d/.test(message.ts) ? message.ts.slice(11, 19) : "--:--:--";
            return <article key={`terminal-${room}-${message.seq}`}><div><span className="terminal-seq">[{String(message.seq).padStart(6, "0")}]</span><time>{timestamp}</time><span className={didFormatted ? "terminal-trust did" : "terminal-trust"}>{didFormatted ? "DID-FORMAT" : "CLAIMED"}</span><b>{short(message.from)}</b></div><p>{message.text}</p></article>;
          })}</div><footer>This terminal renders only the currently loaded observed window. It is not complete history, signature verification, identity proof, or eligibility evidence.</footer></div>
        </div>
      </section>}
      {viewMode === "feed" && <div className="messages" aria-busy={loading}>{filteredMessages.length === 0 && <div className="empty"><span>◇</span><h2>No matching observed messages</h2><p>This room may be new, inactive, ephemeral, outside the current coverage window, or filtered by live-window search. The optional indexer is not complete history.</p></div>}
        {filteredMessages.map((message) => {
          const didFormatted = message.from.startsWith("did:key:");
